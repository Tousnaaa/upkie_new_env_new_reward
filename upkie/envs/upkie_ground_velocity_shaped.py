...
class UpkieGroundVelocityShaped(UpkieBaseEnv):
    r"""!
    ...
    def __init__(
        ...
        reward: Optional[RewardShaping] = None,
        shm_name: str = "/upkie",
        ...
        reward: RewardShaping = (
            reward if reward is not None else RewardShaping()
        )
        ...
        MAX_TORQUE_LEFT : float = float("inf")
        MAX_TORQUE_RIGHT : float = float("inf")
    ...
    def get_env_observation(self, spine_observation: dict) -> np.ndarray:
        ...
        left_torque = spine_observation["servo"]["left_wheel"]["torque"]
        right_torque = spine_observation["servo"]["right_wheel"]["torque"]
        obs = np.empty(6, dtype=float)
    ...
    def get_reward(
        ...
        return self.reward(
            ...
            left_wheel_torque=observation[4],
            right_wheel_torque=observation[5]
        )
